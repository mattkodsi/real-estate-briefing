...
function validPref(key,value) {
 if(['read','learnedTerms','starEvents','watchPlayers'].includes(key)) return Array.isArray(value)&&value.every(x=>typeof x==='string');
 if(key==='saved') return Array.isArray(value)&&value.every(x=>object(x)&&['key','date','id','title'].every(k=>typeof x[k]==='string')&&['section','market'].every(k=>x[k]==null||typeof x[k]==='string'));
 if(key==='seen') return value===null||(object(value)&&typeof value.date==='string'&&object(value.sig)&&Object.values(value.sig).every(x=>typeof x==='string'));
...
export async function handleProfile(req,rpc) {
 ...
 let b;
 try { const raw=await req.text(); if(new TextEncoder().encode(raw).length>2097152) return reply({error:'too_large'},413); b=JSON.parse(raw); } catch {return reply({error:'invalid_json'},400);}
