...
4. **Synthesize & rank.** The reader's time is fixed; input volume is not. As sources grow, **input scales, output doesn't** — more newsletters must produce *better-corroborated* stories, never more of them.
   ...
   - **Story budget: card the top ~30 by importance; brief the tail. The card/brief split is a RANKING call, not a category call** — it uses the SAME importance order defined below (systemic significance FIRST), not "is it a named-property deal." The ~30 most important stories get full cards; everything newsworthy beyond becomes a one-liner: set `brief: true` with only `title`, `summary` (one short sentence), classification fields, `url`, `locations`, `valueUsd` — no featured, no explainer. The app shows briefs as an "Also today" strip; they still feed the map, ledger, players, and open as full reader pages (so always give a brief its real `url` — the fill loop fetches its text like any story).
     - **Auto-card floor — objective triggers that FORCE a card, checked BEFORE the ~30 budget and never overridden by it.** The holistic ranking decides the discretionary middle; these conditions remove the judgment entirely — a story that trips ANY of them can NEVER be a brief, even on a heavy day and even if carding it pushes the count past 30 (the ~30 is a target for the discretionary *tail*, not a cap that can demote a floor story). A story is auto-carded when ANY of these holds:
     ...
     - **Don't under-fill the budget.** On a heavy day, if 25–30 stories clear the significance bar, card ~30 — leaving a Dimon-on-rates warning, a $2B data-center tax exemption, or a REIT office-portfolio bankruptcy in "Also today" next to the 15th $20M closing is a mis-rank, not economy. A busy day carding only ~18 of 60 means the line was drawn too high — genuine significance got buried.
       - **The card body is FILLED to a target; "Also today" is RELATIVE overflow — NOT an absolute-quality reject stories start in.** The briefing's card section should always be populated to roughly its target (~20–30 on a normal day). The card-vs-brief line is drawn *relative to the day's field*, not against a fixed quality bar: card the best-ranked stories DOWN to the target, and let `brief` hold only (a) what genuinely overflows the target once you have more card-worthy stories than it, plus (b) the always-trivial floor (routine tiny closings, minor leases, one-off blurbs). So the SAME story can be a card at 9am (few peers) and get pushed to "Also today" by 2pm as stronger stories arrive and out-rank it — that downward drift is the system working. **The morning failure mode to kill:** only 15–20 articles have come in, the routine cards ~5 of them, and the briefing sits nearly bare while "Also today" quietly fills. Wrong — early in the day, with few stories in, card MOST of what's above the trivial floor so the briefing reads as a full briefing from the first window; the "Also today" tail should stay short until the day's volume actually exceeds the card target. Fill the card body first from the best available; demote to brief only the genuine overflow or the genuinely trivial.
       - **RE-RANK THE WHOLE SET EVERY RUN — never inherit a prior run's card/brief flags.** This is the failure mode that actually bit: the card set *ossified*. Idempotency (step 1) means keeping story **ids** stable across rebuilds — it does NOT mean freezing each story's `brief` flag. A story correctly briefed at 8am can become a card by noon as the day's context grows (a second outlet corroborates it, a bigger peer arrives, it becomes an arc beat). So on every run, re-derive `featured`/card/`brief` for the FULL current story list from scratch by the step-4 ranking — do not just append new arrivals as briefs onto a locked-in early card set. The reader watching all day should see the card body and Top Stories genuinely evolve as the big stories land, not stay static while significant news piles up as footnotes.
       - **The card count SCALES with volume — the ~30 is the *tail* target, never a cap.** ~30 fits a normal ~40–50-story day. On an 80–90-story day, expect **~50–65 cards**: the discretionary tail stays ~30, but every story that clears the significance/novelty/influence bar cards ON TOP of that. Carding 41 of 88 (leaving a $15B data-center sale, a $5.4B hyperscaler financing, and the whole AI-buildout run as footnotes) is the exact mis-rank to avoid — those trip the floor on scale, sector-arc, and prominence. When volume doubles, the card body grows; it does not hold at 30 while briefs balloon.
       - **Ranking is holistic, NOT a dollar threshold.** Big money is *one* signal among systemic significance, breadth, novelty, influence, continuity, and prominence (step-4 order) — a genuinely novel or influential story cards even with no eye-popping number (robot-valet towers, a sector-defining moratorium, a marquee firm's earnings turn), and a routine $30M closing stays a brief. Don't card on "ooo money" alone, and don't brief a defining story just because it lacks a single figure.
