...
- Bisnow — senders on `bisnow.com` (search the domain; they mail from subdomains). The owner subscribes to MANY Bisnow newsletters — most asset classes, most of NYC, Philadelphia, and DC — with mixed cadence (daily / weekly / biweekly / occasional). These are volume feeds: apply the story budget and market tiers (step 4) hard — most Bisnow items belong in `brief` one-liners, the record layer, or as corroboration merged into existing stories, NOT as new full stories. Bisnow articles fetch free (no hard paywall observed); if some content turns out gated, a session cookie can be stored with `python3 scripts/trd_session.py --cookie --domain bisnow.com`. Skip their event promos and sponsored/"Studio B" content entirely.
  - **Bisnow URLs — use the real email link, NEVER fabricate one, and NEVER store the `links.bisnow.com` wrapper.** Bisnow article links are `a-prod.bisnow.io/s/<hex-id>` short-links (e.g. `a-prod.bisnow.io/s/6a5aa4956aed4`, `<id>` always lowercase hex) that 302-redirect to the real `bisnow.com/<market>/news/...` article — copy that short-link verbatim as the story's `url` (the fill loop follows the redirect via the Supabase proxy and stores the canonical page; the proxy fetches Bisnow fine, so a correct short-link fills like any article).
    - **CRITICAL — where the short-link lives depends on the email type.** Bisnow's regular articles carry the `a-prod.bisnow.io/s/<hex>` link in the HTML. But Bisnow's **DIGEST** editions ("CRE's Biggest Stories", the market deal-sheets — the bulk of the volume) put the real `a-prod.bisnow.io/s/<hex>` short-links in the **`text/plain` (PLAINTEXT) part ONLY**; their HTML uses opaque `links.bisnow.com/u/click?...` Iterable click-tracking wrappers that are **byte-corrupted, undecodable, and DEAD** — clicked outside the email they return "Can't find the project that sent this link." So for digest items **parse the PLAINTEXT for the `a-prod.bisnow.io/s/<hex>` link** (same pattern as CRE Daily's beehiiv plaintext), never the HTML href.
    - **NEVER store a `links.bisnow.com/...` url** — it is a dead wrapper, not an article; `fetch_article.is_dead_tracking_wrapper` now hard-rejects `links.bisnow.com` and `link.mail.beehiiv.com`. If you can only find the wrapper, leave `url` unset (summary-only) rather than shipping a broken "Read at source". Same for a fabricated `/s/<non-hex-slug>` (rejected by `is_fabricated_bisnow_shortlink`). If a canonical `bisnow.com/...` article URL appears, that's also fine.
...
4. **Synthesize & rank.** The reader's time is fixed; input volume is not. As sources grow, **input scales, output doesn't** — more newsletters must produce *better-corroborated* stories, never more of them.
   - **Merge, don't multiply.** Dedupe into ONE story list: the same story in N newsletters → one entry, every outlet in `sources`. Corroboration is a ranking signal (a story three desks covered outranks a similar one desk covered); take each fact from the best version and prefer the most authoritative numbers. The main `content`/`url`/`publisher` hold the PRIMARY version (fullest text, most authoritative outlet).
     - **EVERY non-primary outlet MUST get its own `coverage` entry** (schema below) — `{publisher, url, title, content|null, note}` — so the reader can always reach it. This is NOT optional and NOT only for outlets with a different take: an outlet credited in `sources` but absent from `coverage` becomes unreachable (the app renders coverage rows only for entries that have a `url` or `content`). So capture each other outlet's OWN article `url` and `title`; set `content` to its text only when you fetched a genuinely fuller/different version, else null; set `note` to a ≤10-word phrase for what it adds ("adds construction financing details") or **null when it's just the same story** (a null-note row still shows as a link-out). The ONLY outlet that legitimately has no `coverage` row is one whose link is identical to the primary's `url` (pure aggregator pointer — nothing separate to reach). The app renders these as "Also covered by" rows in the reader.
     - **The bar for HIDING is a true duplicate, nothing less.** If two ingested items are the SAME article (a re-report / near-identical restatement, e.g. Bisnow morning + afternoon of one story), keep the better-formatted one as the story and DROP the redundant one entirely (don't publish it as a second story). But a genuine second-outlet take is NEVER dropped — it merges into `coverage` as above. Redundant coverage must never populate the feed, sagas, or map as its own tile; genuine coverage must always remain reachable under the primary article.
   ...
   - **The card/brief line has LOW stakes by design — never confuse it with dropping a story.** A brief is not a lesser story: it renders in "Also today," opens as a full reader page, gets content-filled, and feeds the map, the ledger, the players roster, the Desk comps, and search exactly like a card. So a mis-brief is a *recoverable demotion* — the reader can still find it. The ONLY unrecoverable move in this whole pipeline is DROPPING a story (step above), and that is permitted **only for a true duplicate**. Keep these two decisions separate in your head: "card vs brief" is presentation and forgiving; "keep vs drop" is permanent and reserved for genuine dupes. When unsure whether something is a dup, it is NOT — keep it (as at least a brief).
   - **Story sort: a fixed-capacity revolving leaderboard.** The card ("briefing") section is the **top `CARD_CAP` stories by importance, in strict rank order** — the entire first page is that ranked list. Everything below rank `CARD_CAP` is `brief: true` ("Also today"), also in rank order. `CARD_CAP` is the card capacity (currently **30** — a fixed target, NOT a per-day-scaling number; see "How CARD_CAP is set" below). The mechanism, run EVERY window:
     1. **Rank the full set.** Build ONE strict importance ordering (a total order) of ALL of today's stories by the step-4 order below (systemic significance FIRST … prominence LAST). Re-derive this from scratch every run — ids stay stable (step 1), ranks do not.
     2. **Fill the leaderboard.** The top `CARD_CAP` are cards; the rest are briefs. Render both in rank order (cards top-to-bottom by importance; "Also today" likewise).
     3. **Revolving displacement.** Because you re-rank the whole set each run, a newly-arrived story that outranks the current weakest card automatically takes a card slot and pushes that weakest card DOWN to "Also today." **A card is retired down ONLY because a materially better story needs its slot — never otherwise.** This is the guarantee the sort exists for: *everything carded outranks everything in "Also today."*
     4. **Below capacity, card everything (the morning fix).** If fewer than `CARD_CAP` non-trivial stories exist yet, they are ALL cards — the briefing is never left bare while "Also today" fills. "Also today" only starts growing once the day's real volume exceeds `CARD_CAP`.
     5. **The one absolute floor:** genuinely trivial items (routine tiny digest closings, minor leases, one-off blurbs) stay `brief` even under capacity — don't manufacture cards from pure trivia to hit `CARD_CAP`; on a thin day the card count is simply however many non-trivial stories exist.
     A brief carries only `title`, `summary` (one short sentence), classification fields, `url`, `locations`, `valueUsd` — no featured, no explainer. Briefs still feed the map, ledger, players, Desk, and open as full reader pages (so always give a brief its real `url`).
     - **WRITE `stories[]` IN RANK ORDER (best first).** The app has no separate rank field — it reads each story's **position in the `stories` array as its importance rank**, and orders the feed WITHIN every group (and the Top Stories band, and "Also today") by that position. So the array you emit *is* the leaderboard: sort it strictly by the step-1 importance order (most important first, cards then briefs), every run. Get the array order right and the whole page — group internals included — reads top-to-bottom by importance automatically.
     - **How `CARD_CAP` is set — it does NOT vary per day.** It is a fixed capacity (30). It is a *target the leaderboard fills to*, not a hard ceiling the ranking may never cross and not a floor that forces trivia in: a thin day yields fewer than 30 cards (only what's non-trivial), and the count never balloons past 30 on a heavy day — the extra volume flows into "Also today," which is exactly correct because those stories rank below the top 30. (This supersedes any earlier "card count scales with volume to 50–65" guidance — that was wrong; scale is a *ranking* signal that lifts a big story INTO the top 30, not a reason to grow the page.)
     - **Top-of-leaderboard signals — the marks of a story that belongs in the cards.** These are the objective triggers that place a story HIGH in the ranking (step 1 above), so it lands in the top `CARD_CAP` by construction. A story tripping ANY of them should be a card; if one is sitting in "Also today," that's almost always a ranking inversion to fix (the ONLY legitimate exception: the day genuinely has a full page of even-stronger stories ranked above it — rare). The scan_sort backstop (below) flags exactly this case. The triggers:
       1. **Substantive New York.** `market` is New York AND it is not a routine digest closing (a small Traded/Bisnow/permit blurb) — any NY policy, distress, legal, development, financing, or non-routine sale cards. (A $12M anonymous-borough closing is still a brief; a substantive NY story never is.)
       2. **Market-wide signal.** `dealType` ∈ {Markets, Policy, Distress} carrying a systemic read — a rate/credit signal, a distress-wave print, a sector-reshaping policy, major consolidation — cards even with no single property (ranking rule #1).
       3. **Prominence subject.** The story's SUBJECT (not a bit-part broker/lender credit) is a rostered player, a marquee name/firm, a prominent principal, or an iconic/trophy asset (ranking rule #6).
       4. **Tracked arc.** It carries a `thread` — an installment of a tale the reader is already following; burying an arc's next beat in "Also today" breaks the continuity the tale exists to provide.
       5. **Cited market series.** It cites a recurring industry metric that feeds the ledger (a national rent / vacancy / delinquency / rate print) — a number the reader is tracking over time.
       **The floor is a FLOOR, not a ceiling:** *not* tripping it never forces a brief — most non-floor stories still card if they clear significance; only the routine tail briefs. And after building the day, **run `python3 scripts/scan_sort.py <date>`** (reads Supabase only, like the other scanners): it lists every BRIEF that trips a floor trigger, so a mis-brief is caught mechanically THIS run, not discovered weeks later. Promote each hit to a card — or, rarely, justify leaving it (only a genuine near-dup of a story already carded).
     - **What belongs in briefs:** routine digest closings (small Traded/Bisnow items), minor leases, watch-tier day-to-day deals, and one-off proptech launches / personnel moves / pure data prints. Trivia stays out of the feed — keep this discipline.
     - **What does NOT get briefed — CARD it, even with no single NY property:** a systemically significant story outranks a routine one-asset deal (ranking rule #1). That means a rate/credit signal (a big-bank CEO on CRE credit, a nine-year-low in fundraising, a delinquency-wave print), a **distress** development (a REIT Chapter-11 portfolio selloff, a major foreclosure/bankruptcy), a **sector-reshaping policy** ($B tax programs, housing bonds, financing-rule changes like EB-5/QOZ), or **major consolidation** (a REIT merger, a platform acquisition) is a CARD — a National "trend" or "Markets/Policy/Distress" piece is not automatically a brief. Likewise a genuinely large deal ($200M+) — **or one whose subject is a marquee developer/firm or a landmark/trophy asset** (ranking rule #6, prominence) — is a card, not a brief, wherever it is and whatever the dollar figure: a well-known name or a building every real-estate reader would recognize should not get buried in "Also today."
     - **Ranking is holistic, NOT a dollar threshold.** The single ordering in step 1 weighs systemic significance, scale & irreversibility, breadth, novelty, influence, continuity, and prominence (the step-4 order) — big money is *one* signal, not the ruler. A genuinely novel or influential story ranks high with no eye-popping number (robot-valet towers, a sector-defining moratorium, a marquee firm's earnings turn); a routine $30M closing ranks low. Don't rank on "ooo money" alone, and don't sink a defining story just because it lacks a single figure. (The rank is what the leaderboard reads; get the rank right and the card/brief split is automatic.)
   ...
   6. **Prominence (an elevator, not a tiebreaker)** — a marquee actor or a landmark asset lifts an otherwise-ordinary story to card level on recognizability alone, because "what the big names are doing" and "what's happening to the trophy assets" is exactly what this reader tracks. Elevate a deal/move/lawsuit that is genuinely ABOUT: a top-tier developer/owner/investor or the household names of the industry (e.g. Related, Vornado, SL Green, Brookfield, Blackstone, Tishman Speyer, Extell, Silverstein, RXR, Hines, Durst, Rudin, Fisher Brothers, Witkoff, Macklowe, Zeckendorf, Ken Griffin…), a prominent principal, or an already-rostered player; or an iconic/trophy property (a named tower, a landmark site). A $60M deal on the Woolworth Building, or Related quietly entering a new market, is a card; the same $60M on an anonymous suburban strip is a brief. **Caveat — genuinely about them, a real move:** a bit-part broker/lender credit in a routine digest closing does NOT get elevated just because a big name appears; the story's subject must be the marquee actor or asset.
   **This one ranking draws every line:** the top **3–5** are `featured` (Top Stories), the top **~30** are full cards, and the tail is `brief`. So a story high on systemic significance (a macro/rate signal, a distress wave, a reshaping policy) is a CARD — ahead of a routine one-asset closing — even if it has no NY address; and it anchors the overview + keyPoints. Drawing the card line by deal-type or market instead of by this ranking is the mistake to avoid.
...
7b. **Reader content is filled by the GitHub Actions heartbeat — you don't need working egress.** A scheduled workflow (`.github/workflows/fill-content.yml`, every 30 min) runs `scripts/fill_browser.py` on GitHub's runners: a REAL headless browser (Playwright) that fetches every story still missing content, passes Cloudflare JS challenges, resolves tracking-link redirects to canonical publisher URLs, and republishes the day. So: write each story's `url` (and any content available in the email body itself) and move on — text you can't fetch will fill itself within ~30 minutes. If your sandbox happens to have egress, you MAY still run `python3 scripts/fill_content.py <date> --no-push` as a fast first pass; its failures are expected and non-fatal when egress is blocked (do not spend run time retrying).
   - **Redundant fetch chain (no single point of failure).** Every article is attempted by MULTIPLE methods that fall back automatically — if one is blocked, the next tries, so a wall on one path never silently drops a story: (1) GitHub Actions Playwright (`fill_browser.py`, real browser, passes Cloudflare) → within that, if the browser is challenged it **falls back to plain HTTP + the Supabase proxy** (`fetch_article.extract`) — the path that recovers TRD, which Cloudflare-challenges the GitHub-runner browser but serves fine to a clean-egress fetch; (2) the Supabase `fill-content` edge standby (HTTP, every 15 min); (3) the Mac watchdog (local headless browser); (4) YOU, the routine (this step). When a fetch fails, it is retried by the next method AND on the next window — never abandoned after one try.
   - **Source-health watchdog (every run): `python3 scripts/monitor_sources.py`.** Reads Supabase only (runs in the egress-blocked sandbox). It computes per-publisher fill rates and PROACTIVELY alerts the owner the moment a fetch path stops working — the thing the owner must never discover by accident: (a) a **session-gated site collapsing** (The Real Deal fill-rate cratering = an expired cookie) → it flips the app's reconnect banner AND sends a web push telling the owner to run `scripts/trd_session.py --cookie`; (b) a **normally-reliable source collapsing** (CRE Daily/Bisnow to ~0 = a method broke) → pushes "article fetching is failing." Hard paywalls (GlobeSt/WSJ/Bloomberg/CoStar) are excluded so they never false-alarm. It de-dupes (alerts on the transition, not every run). This also runs automatically after every GitHub Actions fill; run it here too as the cross-check, and if it reports a session/method problem, fold it into the day's `notes`.
   - **Failover duty (every run) — do NOT cry wolf.** The fillers write a pulse to the Supabase `secrets` row `fill_heartbeat` (`GET <SUPABASE_URL>/rest/v1/secrets?id=eq.fill_heartbeat&select=data`). Its `via` field says **which layer wrote it last**, NOT whether anything is broken: `via: "github-actions"` = the Playwright filler; `via: "supabase-edge"` = the pg_cron standby (runs every 15 min, so it is usually the most-recent writer — seeing `supabase-edge` is NORMAL and is **not** evidence GitHub Actions is down). Likewise a single run showing `0 filled / N failed` is **not** a failure — the edge standby legitimately fills 0 when the remaining stories are all hard paywalls (GlobeSt/WSJ/Bloomberg/CoStar). **Judge filler health by ACTUAL COVERAGE, not one pulse:** look at today's stories — if a healthy fraction have `content` (most fetchable ones filled), the pipeline is fine, full stop. Only if today's stories are genuinely NOT filling across multiple windows (few/none gaining text over hours) is there a real problem — then (a) attempt `python3 scripts/fill_content.py <date>` yourself, and (b) add a SHORT note (below). **NEVER write an alarming "GitHub Actions may be down" note off a stale pulse or a `supabase-edge` via — that false alarm already shipped once and must not recur.**
   - **The cloud routine cannot supply `AUDIT_PIPELINE_SECRET` — its own inline fetches failing is EXPECTED, never a note.** The routine's execution environment has no secret-injection mechanism, so once the article proxy and pipeline functions are secured (the backend audit), the routine's OWN `fetch_article.py` / `monitor_sources.py` proxy/push calls raise `AUDIT_PIPELINE_SECRET is required …` and are caught per-item. This is BY DESIGN and fully covered by the three fillers that DO hold the secret — the GitHub Actions Playwright filler (repo Actions secret), the Supabase edge standby (edge env), and the Mac watchdog (plist env). The routine still PUBLISHES normally (`push_data.py` uses the publishable key, needs no secret). So a routine run whose inline fill reports `failed (AUDIT_PIPELINE_SECRET …)` is a **healthy** run — do NOT put it in `notes`, do NOT treat it as a broken fetch method, and do NOT cry wolf.
   - **`notes` is SHORT and USER-FACING — never a debug log.** One or two plain sentences an owner would want ("Inman morning edition didn't arrive"; "TRD session needs a refresh"). ≤ ~240 characters. NO internal jargon (`fill_heartbeat`, `scripts/…py`, "sandbox egress"), NO multi-pass timestamped logs, NO dumping every deduped headline. If nothing is genuinely unusual for the reader, leave `notes` empty. Diagnostics belong in the tooling (`monitor_sources.py`, the status page), not the reader-visible note. (Two more layers also watch this pulse: the `fill-content` Supabase edge function — a pg_cron standby every 15 min that HTTP-fills when the pulse is stale — and a Mac-side watchdog that adds a local headless-browser pass. Your note is the owner's cross-check.) If the pulse is fresh, content-filling needs nothing from you. This is the reliability backstop for step 6: a plain code loop over **every** story that fetches full article text for any still missing it (`content: null` or under ~120 words) via `fetch_article.py` (direct → Supabase proxy → TRD cookie). Fetching articles is therefore **not** a per-story judgment call you can partially skip — this loop guarantees each story is attempted. It is idempotent (stories that already have content are left alone), throttled with one retry pass so tracking-link hosts (beehiiv/Mailchimp) don't rate-limit the batch, and any story that fails transiently is retried automatically on the **next** scheduled window — so coverage self-heals without manual steps. Read its `SUMMARY:` line: if it reports persistent `failed` ids or `TRD-paywalled`, fold that into the day's `notes` (for TRD, refresh the session with `python3 scripts/trd_session.py --cookie`). `--no-push` fills only the local file; step 12's `push_data.py` publishes it.
...
11b. **Collector checklist — do NOT skip (this is the step most often missed).** Steps 10b–10d are mandatory every run, not optional flourishes. Before publishing, confirm you actually built them this run: **threads** (`data/threads.json` — **run `python3 scripts/scan_arcs.py --days 7`** — MERGE its ⚠ duplicates, register genuine progressions from its ■ tale candidates, reject its □ review pairs; any real tale MUST have the linked stories' `thread` fields set), **events** (`data/events.json` — EVERY dated future catalyst mentioned today; a run that produced any `watch` item almost always has events to register), and **metrics** (`data/metrics.json` — run `python3 scripts/scan_metrics.py <date>` and register every recurring series it surfaces; a normal news day yields several, and an empty metrics run on a busy day means you skipped this). Also check **canopies** (step 10b-ii, `data/campaigns.json`) — if today added a sibling initiative to a running agenda (or a second front opened one), the canopy must be created/extended. These are append-only registries, so fetch the current rows from Supabase first (`GET .../threads|campaigns|events|metrics?select=...`) and merge — never overwrite. If a genuinely quiet run has none of a given kind, that's fine — but the default expectation on a normal news day is several events and metrics. (History note: these three registries sat empty for the app's first week because this step was skipped; a one-time backfill mined them from the archive. Don't let them fall behind again.)
    - **Also run the SORT audit: `python3 scripts/scan_sort.py <date>`** (Supabase-only, like the others). It lists every `brief` that trips an auto-card floor trigger (step 4) — a substantive NY story, a market-wide signal, a marquee/rostered subject, a threaded arc-beat, or a cited market series that got left in "Also today." Promote each flagged story to a card (drop `brief`), or justify leaving it only as a genuine near-dup of one already carded. A clean run (no flags) confirms the sort held; this is the mechanical backstop against silently burying relevant stories.
