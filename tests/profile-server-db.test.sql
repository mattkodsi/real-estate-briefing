...
create table public.prefs(profile text primary key,data jsonb not null,updated_at timestamptz);
...
begin
 if has_table_privilege('anon','public.prefs','SELECT') then raise exception 'public prefs readable'; end if;
 ...
 if r::text like '%pinHash%' or r->'profiles'->0->>'hasPin'<>'true' then raise exception 'metadata unsafe'; end if;
 r:=public.reader_profile_api('{"action":"login","profile":"amy","pin":"9999"}','test');
 if r->>'error'<>'unauthorized' then raise exception 'wrong PIN accepted'; end if;
 ...
 if r->>'error'<>'unauthorized' then raise exception 'cross profile session accepted'; end if;
 ...
 if r->'data'->>'theme'<>'dark' or r->'data'->'saved'<> '["old"]'::jsonb then raise exception 'patch clobbered unrelated values'; end if;
 r:=public.reader_profile_api(jsonb_build_object('action','patch','profile','amy','token',t,'changes',jsonb_build_object('pinHash','x')));
 if r->>'error'<>'invalid_changes' then raise exception 'security field patch accepted'; end if;
 ...
 if r->>'error'<>'unauthorized' then raise exception 'revoked token accepted'; end if;
 for i in 1..11 loop r:=public.reader_profile_api('{"action":"login","profile":"amy","pin":"9999"}','different-'||i); end loop;
 if r->>'error'<>'rate_limited' then raise exception 'profile brute force limit bypassed'; end if;
end $$;
rollback;
